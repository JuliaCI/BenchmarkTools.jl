...
push!(trial1, 2, 2, 1, 4, 5)
push!(trial1, 21, 21, 0, 41, 51)

trial2 = BenchmarkTools.Trial(BenchmarkTools.Parameters(time_tolerance = 0.15))
push!(trial2, 21, 21, 0, 41, 51)
push!(trial2, 2, 2, 1, 4, 5)
...
@test trial1.realtimes == trial2.realtimes == [2.0, 21.0]
@test trial1.cputimes == trial2.cputimes == [2.0, 21.0]
...
@test trial1[2] == push!(BenchmarkTools.Trial(BenchmarkTools.Parameters(evals = 2)), 21, 21, 0, 4, 5)
@test trial1[1:end] == trial1

@test realtime(trial1) == realtime(trial2) == 2.0
@test cputime(trial1)  == cputime(trial2)  == 2.0
...
for _ in 1:40
    push!(randtrial, rand(1:20), 1, 1, 1, 1)
end

while mean(randtrial) <= median(randtrial)
    push!(randtrial, rand(10:20), 1, 1, 1, 1)
end
...
@test realtime(tmin) == realtime(randtrial)
...
ta = BenchmarkTools.TrialEstimate(BenchmarkTools.Parameters(), rand(), rand(), rand(), rand(Int), rand(Int))
tb = BenchmarkTools.TrialEstimate(BenchmarkTools.Parameters(), rand(), rand(), rand(), rand(Int), rand(Int))
tr = ratio(ta, tb)

@test realtime(tr) == ratio(realtime(ta), realtime(tb))
@test cputime(tr) == ratio(cputime(ta), cputime(tb))
...
@test BenchmarkTools.gcratio(ta) == ratio(gctime(ta), realtime(ta))
@test BenchmarkTools.gcratio(tb) == ratio(gctime(tb), realtime(tb))

@test BenchmarkTools.cpuratio(ta) == ratio(cputime(ta), realtime(ta))
@test BenchmarkTools.cpuratio(tb) == ratio(cputime(tb), realtime(tb))

##################
# TrialJudgement #
##################

ta = BenchmarkTools.TrialEstimate(BenchmarkTools.Parameters(time_tolerance = 0.50, memory_tolerance = 0.50), 0.49, 0.49, 0.0, 2, 1)
tb = BenchmarkTools.TrialEstimate(BenchmarkTools.Parameters(time_tolerance = 0.05, memory_tolerance = 0.05), 1.00, 1.00, 0.0, 1, 1)
tr = ratio(ta, tb)
tj_ab = judge(ta, tb)
tj_r = judge(tr)

@test ratio(tj_ab) == ratio(tj_r) == tr
@test realtime(tj_ab) == realtime(tj_r) == :improvement
@test cputime(tj_ab) == cputime(tj_r) == :improvement
...
@test realtime(tj_ab_2) == realtime(tj_r_2) == :invariant
@test cputime(tj_ab_2) == cputime(tj_r_2) == :invariant
