...
2. The metrics used to characterize benchmark results and to detect regressions were statistically vulnerable to noise (i.e. not robust).
...
1. Benchmark execution parameters are configured separately from the execution of the benchmark itself. This means that subsequent experiments use more consistent strategies, and avoids branching "substrategies" based on individual samples.
2. A variety of simple characterization metrics are supported, and the user can pick which one to use for regression detection.
...
5. The API is simpler, more transparent, and focused on reducing the effort required to perform common operations.

## Usage Guide

#### The Benchmarking Strategy

Executing a benchmark using BenchmarkTools has two steps:
...
BenchmarkTools.Trial:
  ...
  mean time:       13.14 ns (0.0% GC)
  maximum time:    23.0 ns (0.0% GC)
```
...
#### `Trial`, `TrialEstimate`, `TrialRatio`, `TrialJudgement`
