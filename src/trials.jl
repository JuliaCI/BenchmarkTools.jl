...
Base.time(t::TrialMetrics) = t.time
gctime(t::TrialMetrics) = t.gctime
memory(t::TrialMetrics) = t.memory
allocs(t::TrialMetrics) = t.allocs
...
function ratio(a::Trial, b::Trial)
    return TrialMetrics(ratio(time(a),   time(b)),
...
function TrialJudgement(ratio::TrialMetrics, tolerance = DEFAULT_TOLERANCE)
...
judge(ratio::TrialMetrics, tolerance = DEFAULT_TOLERANCE) = TrialJudgement(ratio, tolerance)
...
function Base.show(io::IO, t::TrialMetrics)
    println(io, "BenchmarkTools.TrialMetrics: ")
...
compactshow(io::IO, t::Trial) = print(io, "Trial(", prettytime(time(t)), ")")
compactshow(io::IO, t::TrialMetrics) = print(io, "TrialMetrics(", prettypercent(time(t)), ")")
compactshow(io::IO, t::TrialJudgement) = print(io, "TrialJudgement(", prettydiff(time(ratio(t))), " => ", time(t), ")")
